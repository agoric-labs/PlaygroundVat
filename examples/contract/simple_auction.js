...
/*
 * A smart contract for running auctions. Simple "first price sealed
 * bid" auction: bids are sealed, highest bidder wins and pays his/her
 * price.
 *
 * Auctions are hosted by a third-party auctioneer, trusted by seller
 * and bidders.  Seller and bidders are mutually suspicious and do not
 * need to trust each other.
 *
 * Inspired by the paper "Distributed Electronic Rights in
 * JavaScript":
 * https://ai.google/research/pubs/pub40673
 *
 * and the escrow exchange contract as first presented by Mark S. Miller in
 * the presentation "two phase commit among strangers":
 * https://www.youtube.com/watch?v=WG0JuONIE-c&list=PLKr-mvz8uvUgybLg53lgXSeLOp4BiwvB2&index=24&t=0s
 * https://github.com/erights/slides/raw/master/2phase-commit-among-strangers.pdf
 *
...
define('contract/simple_auction', ['Q'], function(Q) {
  ...
  var makeAuctioneer = function() {
    ...
    return def({
       ...
       * seatP  = promise for the right to play an exchange contract
       *          trading money for the auctioned good
       */
      makeAuction: function(minBid, stopP, seatP) {
        ...
        // when seller resolves the stop promise, it's time to stop
        // the auction
        var outcomeP = Q(stopP).then( function(_) {
          // revoking the auctionToken ensures that any later bids
          // will fail
      ...
      /**
       * auctionToken = token that allegedly identifies an auction
       *                hosted by this auctioneer.
  ...
  var makeSeller = function() {
    return def({
       ...
       * auctioneer = a reference to the auctioneer on which to host
       *              the auction
       */
      sell: function(good, minimumBid, timeout, purse, auctioneer) {
        ...
        var pairP = Q(auctioneer).send('makeAuction', minimumBid,
                                       Q.delay(timeout), seatP);
        pairP.then( function(tuple) {
          ...
          Q(outcomeP).then(
            function(maxBid) {
              // there was a bidder, willing to pay maxBid amount of
              // money for the good set up escrow exchange
  ...
  var makeBidder = function() {
    return def({
      ...
      /**
       * goodDesc = a description of the good the bidder is interested
       *            in. This description should allow the escrow
       *            exchange contract to check that the auctioned 
       *            good is indeed the good expected by the bidder
       * myBid = the amount of money the bidder is willing to bid
       * myPurse = the purse from which to pay the seller when winning
       *           the auction, must contain at least myBid amount of money
       * auctionToken = a token that uniquely identifies the auction
      ...
      bid: function(goodDesc, myBid, myPurse, auctionToken, auctioneer) {
        ...
        Q.then(Q(auctioneer).send('placeBid', auctionToken, myBid),
           function(seatP) {
             // won the bid: exchange myBid money for the advertised
             // good via the seat
           ...
           function(reason) {
             // auctionToken is invalid (auction may have already
             // finished), or, lost the bid (in which case it makes
             // sense to try again with a higher bid)
           }
        );
