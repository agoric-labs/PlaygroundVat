...
export function escrowExchange(a, b) {          // a from Alice, b from Bob
  ...
  const transfer = function(srcPurseP, dstPurseP, amount) {
    const makeEscrowPurseP = Q.join(Q(srcPurseP).invoke('getMakePurse'),
                                  Q(dstPurseP).invoke('getMakePurse'));
    const escrowPurseP = Q(makeEscrowPurseP).fcall();

    Q(decisionP).then(                                    // setup phase 2
      function(_) { Q(dstPurseP).invoke('deposit', amount, escrowPurseP); },
      function(_) { Q(srcPurseP).invoke('deposit', amount, escrowPurseP); });

    return Q(escrowPurseP).invoke('deposit', amount, srcPurseP);  // phase 1
  };

  const failOnly = function(cancellationP) { return Q(cancellationP).then(
